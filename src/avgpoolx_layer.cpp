...
avgpoolx_layer make_avgpoolx_layer(int batch, int h, int w, int c, int size, int stride, int padding)
{
    ...
    l.forward = forward_avgpoolx_layer;
    l.backward = backward_avgpoolx_layer;
    
    #ifdef GPU
    error("avgpoolx has not implemented for gpu yet!");
    // l.forward_gpu = forward_avgpool_layer_gpu;
    // l.backward_gpu = backward_avgpool_layer_gpu;
    // l.output_gpu  = cuda_make_array(l.output, output_size);
    // l.delta_gpu   = cuda_make_array(l.delta, output_size);
    #endif
    fprintf(stderr, "avgx          %d x %d / %d  %4d x%4d x%4d   ->  %4d x%4d x%4d\n", size, size, stride, w, h, c, l.out_w, l.out_h, l.out_c);
    return l;
}
#endif
    ...
    for(b = 0; b < l.batch; ++b){
        for(k = 0; k < c; ++k){
            for(i = 0; i < h; ++i){
                for(j = 0; j < w; ++j){
                    int out_index = j + w*(i + h*(k + c*b));
                    float val = l.delta[out_index]/(l.size*l.size);
                    for(n = 0; n < l.size; ++n){
                        for(m = 0; m < l.size; ++m){
                            int cur_h = h_offset + i*l.stride + n;
                            int cur_w = w_offset + j*l.stride + m;
        ...
        for(k = 0; k < c; ++k){
            for(i = 0; i < h; ++i){
                for(j = 0; j < w; ++j){
                    ...
                    //l.output[out_index] = sum / (l.size*l.size);
                    l_output[out_index] = sum / (l.size*l.size);
                }
            }
        }
    }
        ...
        for(k = 0; k < c; ++k){
            for(i = 0; i < h; ++i){
                for(j = 0; j < w; ++j){
                    int out_index = j + w*(i + h*(k + c*b));
                    //float val = l.delta[out_index]/(l.size*l.size);
                    float val = l_delta[out_index]/(l.size*l.size);
                    for(n = 0; n < l.size; ++n){
                        for(m = 0; m < l.size; ++m){
                            int cur_h = h_offset + i*l.stride + n;
                            int cur_w = w_offset + j*l.stride + m;
